(*
	...
	(C) 2010-2016 Tobias Elze, modified for tesseract Heinrich Schwietering 2012
...
let run ?(crash=true) cmd =
	...
	(
		...
		if crash  then
		(
			prerr_endline ("Terminating create_torrent. All temporary files are kept.");
			exit 2;
		)
	)
;;
...
let process_ocr 
	...
	let process_page (curr_page, pdfname) =
		let tmppicfile = Filename.temp_file "create_torrent" (if rgb then ".ppm" else if gray then ".pgm" else ".pbm") in
		let tmptessinpfile = Filename.temp_file "create_torrent" ".tif" in
		let tmpocrfile = Filename.temp_file "create_torrent" "" in
		let tmpcolfigfile = Filename.temp_file "create_torrent" "_col.png" in
		let tmprescaled_infile = Filename.temp_file "create_torrent" "_rescaled.pdf" in
		let tmpunpaperfile = Filename.temp_file "create_torrent" ("_unpaper" ^ (if rgb then ".ppm" else if gray then ".pgm" else ".pbm")) in
		if not !quiet then
		...
		(
			if not !quiet then
				print_endline "Tesseract was unable to produce a pdf ouput file. Possibly, version of tesseract is prior to 3.03 and cannot output pdf yet. Using hocr2pdf instead.";
	...
	let tmppdf_arr = 
		Array.init pages_to_process (fun i -> (i+first_page, Filename.temp_file "create_torrent" ".pdf"))
	in
	...
	let tmpoutfile = 
		...
		(
			let tout = Filename.temp_file "pdfsandwich_output" ".pdf" in
...
let main () =
	...
	let speclist = [
		...
		("-version", Arg.Unit (fun () -> Printf.printf "create_torrent version %s\n" pdfsandwich_version; exit 0), "\t print version and quit");
	] in
	
	Arg.parse speclist (fun s -> arg_filename := s) "USAGE: create_torrent [options] inputfile.pdf\n\nOptions:";
	...
	pr ("create_torrent version " ^ pdfsandwich_version);
	...
	let internal_input_filename = Filename.temp_file "pdfsandwich_inputfile" ".pdf" in
